...
GlobSync2.prototype.getFilesFromDir = function(dir, fileList) {
  var filesToReturn = [];
  function walkDir(currentPath) {
    var files = fs.readdirSync(currentPath);
    for (var i in files) {
      var curFile = path.join(currentPath, files[i]);
      if (fs.statSync(curFile).isFile() && fileList.indexOf(files[i]) != -1) {
...
GlobSync2.prototype.getFileFromDir = function(dir, file) {
  var fileToReturn;
  var fileFound = false;
  function walkDir(currentPath) {
    if(fileFound) return;
    ...
    for (var i in files) {
      var curFile = path.join(currentPath, files[i]);
      if (fs.statSync(curFile).isFile() && file == files[i]) {
        fileToReturn = curFile;//.replace(dir, ''));
        fileFound = true;
        return
      } else if (fs.statSync(curFile).isDirectory()) {
       walkDir(curFile);
